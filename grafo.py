...
class Grafo(object):
    ...
    def __init__(self, es_dirigido = False):
		...
		if not isinstance(es_dirigido,bool):
			raise TypeError("{} no es un valor booleano".format(es_dirigido))
    ...
    def __delitem__(self, id):
		...
		if id not in self.grafo:
			raise KeyError("El vértice {} no se encuentra en el grafo.".format(id))
    ...
    def agregar_arista(self, desde, hasta, peso = 1):
       ...
       if desde not in self.grafo or hasta not in self.grafo:
			raise KeyError("Alguno/ambos de los vértices pasados no se encuentra/n en el grafo")
    ...
    def borrar_arista(self, desde, hasta):
		...
		if hasta not in self.grafo[desde]:
			raise ValueError("La arista a borrar no se encuentra en el grafo.")
